...
import org.doubango.tinyWRAP.MessagingMbmsSession;
 ...
 */
public class MyMessagingMbmsSession extends NgnSipSession {
	private static String TAG = Utils.getTAG(MyMessagingMbmsSession.class.getCanonicalName());

	private final MessagingMbmsSession mSession;
	private static int SMS_MR = 0;

	private final static NgnObservableHashMap<Long, MyMessagingMbmsSession> sSessions = new NgnObservableHashMap<Long, MyMessagingMbmsSession>(true);

	public static MyMessagingMbmsSession takeIncomingSession(NgnSipStack sipStack, MessagingMbmsSession session, SipMessage sipMessage){
		final String toUri = sipMessage==null ? null: sipMessage.getSipHeaderValue("f");
		MyMessagingMbmsSession imSession = new MyMessagingMbmsSession(sipStack, session, toUri);
		sSessions.put(imSession.getId(), imSession);
        return imSession;
    }

    public static MyMessagingMbmsSession createOutgoingSession(NgnSipStack sipStack, String toUri){
        synchronized (sSessions){
            final MyMessagingMbmsSession imSession = new MyMessagingMbmsSession(sipStack, null, toUri);
            sSessions.put(imSession.getId(), imSession);
            return imSession;
        }
    }

    public static void releaseSession(MyMessagingMbmsSession session){
		synchronized (sSessions){
    ...
    public static void releaseSession(long id){
		synchronized (sSessions){
			MyMessagingMbmsSession session = MyMessagingMbmsSession.getSession(id);
            if (session != null){
                session.decRef();
                sSessions.remove(id);
            }
	...
	public static MyMessagingMbmsSession getSession(long id) {
		synchronized (sSessions) {
	...
	protected MyMessagingMbmsSession(NgnSipStack sipStack, MessagingMbmsSession session, String toUri) {
		super(sipStack);
        mSession = session == null ? new MessagingMbmsSession(sipStack) : session;
	...
	public boolean SendBinaryMessage(String text, String SMSC){
        ...
        if ((SMSCPhoneNumber = NgnUriUtils.getValidPhoneNumber(SMSC)) != null && (dstPhoneNumber = NgnUriUtils.getValidPhoneNumber(dstSipUri)) != null){
            ...
            super.addHeader("Accept-Contact", "urn:urn-7:3gpp-service.ims.icsi.mcptt;require;explicit");
            super.addHeader("P-Preferred-Service", "urn:urn-7:3gpp-service.ims.icsi.mcptt");
            ...
            //}
            //else{
                    rpMessage = SMSEncoder.encodeSubmit(++MyMessagingMbmsSession.SMS_MR, SMSCPhoneNumber, dstPhoneNumber, text);
            //}
            ...
            rpMessage.delete();
            if(MyMessagingMbmsSession.SMS_MR >= 255){
                 MyMessagingMbmsSession.SMS_MR = 0;
            }
