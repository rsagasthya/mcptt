...
tsk_param_t *tsk_params_parse_param(const char* line, tsk_size_t size)
{
	if(line && size){
		...
		if (param && equal && equal<end) {
			...
			if ((param->name = (char*)calloc((tsk_size_t)((equal - start) + 1), sizeof(const char)))) {
			...
			#if HAVE_CRT //Debug memory
			if ((param->value = (char*)calloc((tsk_size_t)((end - equal - 1) + 1), sizeof(const char)))) {
			...
			#if HAVE_CRT //Debug memory
			if ((param->name = (char*)calloc((tsk_size_t)((end - start) + 1), sizeof(const char)))) {
