...
int tsip_header_Authorization_serialize(const tsip_header_t* header, tsk_buffer_t* output)
{
	if(header){
		...
		if(Authorization && Authorization->scheme){
			return tsk_buffer_append_2(output, "%s %s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s", 
				...
				Authorization->auts ? "\"" : "",
				Authorization->auts_param ? ",auts-param=\"" : "",
				Authorization->auts_param ? Authorization->auts_param : "",
				Authorization->auts_param ? "\"" : "",
				Authorization->auts_value ? ",auts-value=\"" : "",
				Authorization->auts_value ? Authorization->auts_value : "",
				Authorization->auts_value ? "\"" : ""
				);
		}
	}
	return -1;
...
tsip_header_Authorization_t *tsip_header_Authorization_parse(const char *data, tsk_size_t size)
{
	...
	if((hdr_http = thttp_header_Authorization_parse(data, size))){
		...
		//hdr_sip->auts_value =  tsk_strdup(hdr_http->auts_value);
...
static tsk_object_t* tsip_header_Authorization_dtor(tsk_object_t *self)
{
	...
	if(Authorization){
		...
		TSK_FREE(Authorization->auts_value);
