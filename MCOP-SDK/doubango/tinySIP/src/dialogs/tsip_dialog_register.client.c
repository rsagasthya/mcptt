...
/* ======================== transitions ======================== */
/*static*/ int tsip_dialog_register_msession_start(tsip_dialog_register_t *self);
...
{//Is register.
	...
	*/
		...
		tsk_list_foreach(item, TSIP_DIALOG_GET_STACK(self)->associated_uris){
			...
			if(tsk_object_cmp(TSIP_DIALOG_GET_STACK(self)->identity.preferred, uri)==0){
				barred = 0;
				break;
			}
		}
...
int tsip_dialog_register_msession_start(tsip_dialog_register_t *self)
{
