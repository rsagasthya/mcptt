...
int32_t MediaSessionMgr::sessionHold(twrap_media_type_t media)
{
	int32_t value = 0;
	tmedia_type_t _media = twrap_get_native_media_type(media);
	(tmedia_session_mgr_hold(m_pWrappedMgr, _media));
	return value;
}

int32_t MediaSessionMgr::sessionResume(twrap_media_type_t media)
{
	int32_t value = 0;
	tmedia_type_t _media = twrap_get_native_media_type(media);
	(tmedia_session_mgr_resume(m_pWrappedMgr, _media, tsk_true));
	return value;
}
...
bool MediaSessionMgr::defaultsSetPoCQoE(tmedia_poc_qoe_profile_t profile, tmedia_poc_qoe_profile_strength_t strength) {
	return (tmedia_defaults_set_poc_qoe(profile, strength) == 0);
}