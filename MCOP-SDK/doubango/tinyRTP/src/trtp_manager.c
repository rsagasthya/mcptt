...
int trtp_manager_prepare(trtp_manager_t* self)
{
	...
	else if(self->is_multicast)
	{
		...
		TSK_DEBUG_INFO("RTP/RTCP manager[Begin]: Trying to bind to multicast port %d", local_port);

		/* RTP */
		if(!(self->transport = tnet_transport_create(TNET_SOCKET_HOST_ANY, local_port, socket_type, TRTP_TRANSPORT_NAME))){
			TSK_DEBUG_ERROR("Failed to create RTP/RTCP Transport");
			return -3;
		}

		TSK_DEBUG_INFO("RTP/RTCP manager[End]: Trying to bind to multicast port %d", local_port);

		rtp_local_ip = self->transport->master->ip;
		rtp_local_port = self->transport->master->port;
		self->use_rtcp = tsk_false;
		self->use_rtcpmux = tsk_false;
		/*
		...
		TSK_DEBUG_INFO("RTP/RTCP manager[Begin]: Trying to join multicast IP %s", self->multicast.multicast_ip);

		if(!self->use_ipv6) //Check IPv6
		{
			struct sockaddr_in multicast_addr; 
			if(tnet_inet_pton(AF_INET, self->multicast.multicast_ip, &(multicast_addr.sin_addr)) > 0) 
			{
				int err = -1;
				tsk_bool_t use_idx = tsk_false;
				struct ip_mreq mreq;
				tnet_address_t* address = tsk_null;
#ifdef _WIN32
				mreq.imr_multiaddr.s_addr = multicast_addr.sin_addr.S_un.S_addr;
				if (self->multicast.multicast_iface &&
					!tsk_strempty(self->multicast.multicast_iface)) {
#else
				mreq.imr_multiaddr.s_addr = multicast_addr.sin_addr.s_addr;
				if (self->multicast.multicast_iface_idx == -1 &&
					self->multicast.multicast_iface &&
					!tsk_strempty(self->multicast.multicast_iface)) {
					int index = 0;
					if ((index = if_nametoindex(self->multicast.multicast_iface)) != 0) {
						TSK_DEBUG_INFO("Obtained interface %s index: %d", self->multicast.multicast_iface, index);
						self->multicast.multicast_iface_idx = index;
					}
				}

				if (self->multicast.multicast_iface_idx != -1) {
					struct ip_mreqn mreqn;
					TSK_DEBUG_INFO("RTP/RTCP manager[Begin]: Trying to join multicast IP %s in interface index %d", self->multicast.multicast_ip, self->multicast.multicast_iface_idx);
					mreqn.imr_multiaddr.s_addr = multicast_addr.sin_addr.s_addr;
					mreqn.imr_ifindex = self->multicast.multicast_iface_idx;
					err = setsockopt(self->transport->master->fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char*)&mreqn, sizeof(mreqn));
					if (err < 0) {
						TSK_DEBUG_ERROR("Failed to add socket to multicast group");
						return -3;
					}
					use_idx = tsk_true;
					TSK_DEBUG_INFO("RTP/RTCP manager[End]: Trying to join multicast IP %s in interface index %d", self->multicast.multicast_ip, self->multicast.multicast_iface_idx);					
				} else if (self->multicast.multicast_iface &&
						   !tsk_strempty(self->multicast.multicast_iface)) {
#endif
					TSK_DEBUG_INFO("RTP/RTCP manager[Begin]: Trying to join multicast IP %s in interface %s", self->multicast.multicast_ip, self->multicast.multicast_iface);
					address = tnet_get_interface_addr(self->multicast.multicast_iface, AF_INET);
					if (address != tsk_null && address->ip != tsk_null) {
						TSK_DEBUG_ERROR("IP address of interface %s: %s", self->multicast.multicast_iface, address->ip)
						mreq.imr_interface.s_addr = inet_addr(address->ip);
					} else if (self->local_ip != tsk_null) {
						TSK_DEBUG_ERROR("Could not find IP address for interface %s", self->multicast.multicast_iface)
						mreq.imr_interface.s_addr = inet_addr(self->local_ip);
					} else
						mreq.imr_interface.s_addr = INADDR_ANY;
					TSK_DEBUG_INFO("RTP/RTCP manager[End]: Trying to join multicast IP %s in interface %s", self->multicast.multicast_ip, self->multicast.multicast_iface);
				} else if (self->local_ip != tsk_null)
					mreq.imr_interface.s_addr = inet_addr(self->local_ip);// If INADDR_ANY is set, the kernel selects the interface, and it was not selecting the correct one
				else
					mreq.imr_interface.s_addr = INADDR_ANY;

#ifndef _WIN32
				if (use_idx == tsk_false) {
#endif
				err = setsockopt(self->transport->master->fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char*)&mreq, sizeof(mreq));
				if (err < 0) {
					TSK_DEBUG_ERROR("Failed to add socket to multicast group");
					return -3;
				}
		...
		else
		{
			...
			if(tnet_inet_pton(AF_INET6, self->multicast.multicast_ip, &(multicast_addr.sin6_addr)) > 0) 
			{
				int err = -1;
			    struct ipv6_mreq mreq;
				mreq.ipv6mr_multiaddr = multicast_addr.sin6_addr;
                mreq.ipv6mr_interface = 0;
                err = setsockopt(self->transport->master->fd, IPPROTO_IPV6, IPV6_JOIN_GROUP, (char*)&mreq, sizeof(mreq));
                if (err < 0)
                {
                 	TSK_DEBUG_ERROR("Failed to add socket to multicast group");
					return -3;
		...
		TSK_DEBUG_INFO("RTP/RTCP manager[End]: Trying to join multicast IP %s", self->multicast.multicast_ip);
	}
	else{
...
int trtp_manager_disable_automated_rtcp_reporting(trtp_manager_t* self)
{
	if(!self){
		TSK_DEBUG_ERROR("Invalid parameter");
		return -1;
	...
	if(self->rtcp.session){
		return trtp_rtcp_session_disable_automated_reporting(self->rtcp.session);
	}
	...
	if(self->use_rtcp){
		...
		if(self->rtcp.session){
			...
			if(self->is_MCPTT_session==tsk_true){
				...
				self->rtcp.session->is_MCPTT_session=tsk_false;
			}
...
tsk_size_t trtp_manager_send_rtp(trtp_manager_t* self, const void* data, tsk_size_t size, uint32_t duration, tsk_bool_t marker, tsk_bool_t last_packet)
{
	...
	if(!self){
		...
		TSK_DEBUG_ERROR("Invalid parameter 1");
	}else if(!data){
		TSK_DEBUG_ERROR("Invalid parameter 2");
	}else if(!size){
		TSK_DEBUG_ERROR("Invalid parameter 3");
	}
	...
	if((packet->payload.data = calloc(size, sizeof(uint8_t)))){
...
static tsk_object_t* trtp_manager_ctor(tsk_object_t * self, va_list * app)
{
	...
	if(manager){
		...
		/* rtcp */
        {
            ...
            tsk_sprintf(&manager->rtcp.cname, "organization.%llu", (tsk_time_now() + rand()));
