...
typedef struct tmedia_session_plugin_def_s
{
	...
	struct{ /* Special case */
		...
		uint32_t (* get_ssrc) (tmedia_session_t*);
		uint16_t (* get_last_seqnum) (tmedia_session_t*);
	} audio;
...
TINYMEDIA_API uint32_t tmedia_session_audio_get_ssrc(tmedia_session_audio_t* self);
TINYMEDIA_API uint16_t tmedia_session_audio_get_last_seqnum(tmedia_session_audio_t* self);
...
/** MCPTT Session */
struct tmedia_session_mcptt_s;
struct tmcptt_event_s;
struct tmcptt_mbms_event_s;
typedef int (*tmedia_session_mcptt_cb_f)(const struct tmcptt_event_s* event);
typedef int (*tmedia_session_mcptt_mbms_cb_f)(const struct tmcptt_mbms_event_s* event);
typedef struct tmedia_session_mcptt_s
{
	TMEDIA_DECLARE_SESSION;

	struct {
		tmedia_session_mcptt_cb_f func;
		const void* data;
	} callback;
	struct {
		tmedia_session_mcptt_mbms_cb_f func;
		const void* data;
	} callback_mbms;
	int (* request_token) (struct tmedia_session_mcptt_s*, va_list *app);
	int (* release_token) (struct tmedia_session_mcptt_s*, va_list *app);
	int (* start_mbms_manage) (struct tmedia_session_mcptt_s*,const char* remote_ip,const int remote_port, const char* local_iface, const int local_iface_idx, va_list *app);
	int (* stop_mbms_manage) (struct tmedia_session_mcptt_s*, va_list *app);
	int (* start_mbms_media) (struct tmedia_session_mcptt_s*, const char* media_ip, const int media_port, const int media_ctrl_port, va_list *app);//tmedia_session_mgr_t*, va_list *app);
}
tmedia_session_mcptt_t;
TINYMEDIA_API char* tmedia_session_mcptt_get_mbms_ip(tmedia_session_mcptt_t* self);
TINYMEDIA_API char* tmedia_session_mcptt_get_mbms_audio_port(tmedia_session_mcptt_t* self);
#define tmedia_session_mcptt_init(self)	tmedia_session_init(TMEDIA_SESSION(self), tmedia_mcptt)
#define tmedia_session_mcptt_deinit(self) tmedia_session_deinit(TMEDIA_SESSION(self))
#define tmedia_session_mcptt_create() tmedia_session_create(tmedia_mcptt)
...
typedef struct tmedia_session_mcptt_affiliation_s
{
	TMEDIA_DECLARE_SESSION;

	struct {
		tmedia_session_mcptt_affiliation_cb_f func;
		const void* data;
	} callback;
	
	int (* request_token) (struct tmedia_session_mcptt_affiliation_s*, va_list *app);
	int (* release_token) (struct tmedia_session_mcptt_affiliation_s*, va_list *app);
}
tmedia_session_mcptt_affiliation_t;
#define tmedia_session_mcptt_affiliation_init(self)	tmedia_session_init(TMEDIA_SESSION(self), tmedia_mcptt_affiliation)
#define tmedia_session_mcptt_affiliation_deinit(self) tmedia_session_deinit(TMEDIA_SESSION(self))
#define tmedia_session_mcptt_affiliation_create() tmedia_session_create(tmedia_mcptt_affiliation)
#define TMEDIA_DECLARE_SESSION_MCPTT_AFFILATION tmedia_session_mcptt_affiliation_t __session_mcptt_affiliation__
...
typedef struct tmedia_session_mgr_s
{
	...
	/* PoC QoE */
	struct {
		tmedia_poc_qoe_profile_t profile;
		tmedia_poc_qoe_profile_strength_t strength;
	} poc_qoe;
...
TINYMEDIA_API tmedia_session_t* tmedia_session_mgr_find_at_index(tmedia_session_mgr_t* self, tmedia_type_t type, tsk_size_t index);
...
TINYMEDIA_API int tmedia_session_mgr_set_poc_qoe(tmedia_session_mgr_t* self, tmedia_poc_qoe_profile_t poc_qoe_profile, tmedia_poc_qoe_profile_strength_t poc_qoe_profile_strength);
...
TINYMEDIA_API uint32_t tmedia_session_mgr_audio_get_ssrc(tmedia_session_mgr_t* self);
TINYMEDIA_API uint32_t tmedia_session_mgr_audio_get_last_seqnum(tmedia_session_mgr_t* self);
