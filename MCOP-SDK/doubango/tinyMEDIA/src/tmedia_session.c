...
//MCPTT
int tmedia_session_set_mcptt_cbfn(tmedia_session_t* self, const void* usrdata, tmedia_session_mcptt_cb_f fun)
{
	tmedia_session_mcptt_t* mcptt;
	if (!self){
		TSK_DEBUG_ERROR("Invalid parameter");
		return -1;
	}
	mcptt = (tmedia_session_mcptt_t*)self;
	mcptt->callback.func = fun;
	mcptt->callback.data = usrdata;
	
	return 0;
}
int tmedia_session_set_mcptt_mbms_cbfn(tmedia_session_t* self, const void* usrdata, tmedia_session_mcptt_mbms_cb_f fun)
{
	tmedia_session_mcptt_t* mcptt;
	if (!self){
		TSK_DEBUG_ERROR("Invalid parameter");
	...
	mcptt = (tmedia_session_mcptt_t*)self;
	mcptt->callback_mbms.func = fun;
	mcptt->callback_mbms.data = usrdata;
...
int tmedia_session_set_mcptt_affiliation_cbfn(tmedia_session_t* self, const void* usrdata, tmedia_session_mcptt_affiliation_cb_f fun)
{
	...
	mcptt_affiliation = (tmedia_session_mcptt_affiliation_t*)self;
	mcptt_affiliation->callback.func = fun;
	mcptt_affiliation->callback.data = usrdata;
...
uint32_t tmedia_session_audio_get_ssrc(tmedia_session_audio_t* self)
{
	if (!self || !TMEDIA_SESSION(self)->plugin || !TMEDIA_SESSION(self)->plugin->audio.get_ssrc){
		TSK_DEBUG_ERROR("Invalid parameter");
		return -1;
	}
	return TMEDIA_SESSION(self)->plugin->audio.get_ssrc(TMEDIA_SESSION(self));
}

uint16_t tmedia_session_audio_get_last_seqnum(tmedia_session_audio_t* self)
{
	if (!self || !TMEDIA_SESSION(self)->plugin || !TMEDIA_SESSION(self)->plugin->audio.get_last_seqnum){
		TSK_DEBUG_ERROR("Invalid parameter");
		return -1;
	}
	return TMEDIA_SESSION(self)->plugin->audio.get_last_seqnum(TMEDIA_SESSION(self));
}
...
tmedia_session_t* tmedia_session_mgr_find(tmedia_session_mgr_t* self, tmedia_type_t type)
{
	...
	return (tmedia_session_t*)tsk_object_ref(session);
}

tmedia_session_t* tmedia_session_mgr_find_at_index(tmedia_session_mgr_t* self, tmedia_type_t type, tsk_size_t index)
{
	tmedia_session_t* session;

	tsk_list_lock(self->sessions);

	session = (tmedia_session_t*)tsk_list_find_object_by_pred_at_index(self->sessions, __pred_find_session_by_type, &type, index);
	tsk_list_unlock(self->sessions);
...
{//Call functions start in plugind
...
int tmedia_session_mgr_set_ro(tmedia_session_mgr_t* self, const tsdp_message_t* sdp, tmedia_ro_type_t ro_type)
{
	...
	tmedia_type_t M_last_media_type = tmedia_none;
	...
	while ((M = (const tsdp_header_M_t*)tsdp_message_get_headerAt(sdp, tsdp_htype_M, index++))) {
		...
		if(index > 1) {  //(INFO message SDP)
			...
			if(M_current_media_type == M_last_media_type && _tmedia_session_mgr_find_session_at_index(list_tmp_sessions, (index - active_sessions_count)) == tsk_null) {
				tmedia_session_t* session;
				if ((session = tmedia_session_create(M_current_media_type))) {
					if (!session->prepared && M->port && (_tmedia_session_prepare(TMEDIA_SESSION(session)))){
						TSK_DEBUG_ERROR("Failed to prepare session"); /* should never happen */
						goto bail;
					}
					if((ret = _tmedia_session_set_ro(TMEDIA_SESSION(session), M)) == 0)
...
int tmedia_session_mgr_set_poc_qoe(tmedia_session_mgr_t* self, tmedia_poc_qoe_profile_t profile, tmedia_poc_qoe_profile_strength_t strength)
{
	if (!self){
		TSK_DEBUG_ERROR("Invalid parameter");
		return -1;
	...
	self->poc_qoe.profile = profile;
	self->poc_qoe.strength = strength;
	return 0;
}
...
uint32_t tmedia_session_mgr_audio_get_ssrc(tmedia_session_mgr_t* self)
{
	tmedia_session_audio_t* session;
	static const tmedia_type_t audio_type = tmedia_audio;
	int ret = -3;
	...
	if (session){
		session = tsk_object_ref(session);
		ret = tmedia_session_audio_get_ssrc(TMEDIA_SESSION_AUDIO(session));
		TSK_OBJECT_SAFE_FREE(session);
	}
	else{
		TSK_DEBUG_ERROR("No audio session associated to this manager");
...
//MCPTT
int tmedia_session_mgr_set_mcptt_cbfn(tmedia_session_mgr_t* self, const void* context, tmedia_session_mcptt_cb_f fun)
{
	tmedia_session_t* session;
	tsk_list_item_t *item;
	...
	tsk_list_lock(self->sessions);
	tsk_list_foreach(item, self->sessions){
		if (!(session = item->data) || !(session->type & tmedia_mcptt)){
			continue;
		}
		tmedia_session_set_mcptt_cbfn(session, context, fun);
	}
	tsk_list_unlock(self->sessions);
...
int tmedia_session_mgr_request_mcptt_token(tmedia_session_mgr_t* self, ...)
{
	tmedia_session_t* session;
	tmedia_session_mcptt_t* mcptt = tsk_null;
	tmedia_type_t media_type = tmedia_mcptt;
	tsk_list_item_t *item;
	int ret = -1;
	...
	tsk_list_foreach(item, self->sessions){
		...
		mcptt = tsk_object_ref(session);
		if(mcptt->request_token)
			ret = mcptt->request_token(TMEDIA_SESSION_MCPTT(mcptt), &ap);
		TSK_OBJECT_SAFE_FREE(mcptt);
		va_end(ap);
		break;
	}
	tsk_list_unlock(self->sessions);
...
int tmedia_session_mgr_release_mcptt_token(tmedia_session_mgr_t* self, ...)
{
	tmedia_session_t* session;
	tmedia_session_mcptt_t* mcptt = tsk_null;
	tmedia_type_t media_type = tmedia_mcptt;
	...
	tsk_list_foreach(item, self->sessions){
		...
		mcptt = tsk_object_ref(session);
		if(mcptt->release_token)
			ret = mcptt->release_token(TMEDIA_SESSION_MCPTT(mcptt), &ap);
		TSK_OBJECT_SAFE_FREE(mcptt);
		va_end(ap);
		break;
	}
...
/**
Start MBMS session of manager
*/
int tmedia_session_mgr_start_mbms_manager(tmedia_session_mgr_t* self,const char* remote_ip,const int remote_port, const char* local_iface, const int local_iface_idx, ...)
{
	tmedia_session_t* session;
	tmedia_session_mcptt_t* mcptt = tsk_null;
	tmedia_type_t media_type = tmedia_mcptt;
	...
	tsk_list_foreach(item, self->sessions){
		...
		if(mcptt->start_mbms_manage)
			//Execute start mbms manager
			ret = mcptt->start_mbms_manage(TMEDIA_SESSION_MCPTT(mcptt),remote_ip, remote_port, local_iface, local_iface_idx, &ap);
		TSK_OBJECT_SAFE_FREE(mcptt);
		va_end(ap);
		break;
	}
...
/**
Stop MBMS session of manager
*/
int tmedia_session_mgr_stop_mbms_manager(tmedia_session_mgr_t* self, ...)
{
	tmedia_session_t* session;
	tmedia_session_mcptt_t* mcptt = tsk_null;
	tmedia_type_t media_type = tmedia_mcptt;
	...
	tsk_list_foreach(item, self->sessions){
		...
		mcptt = tsk_object_ref(session);
		if(mcptt->stop_mbms_manage)
			ret = mcptt->stop_mbms_manage(TMEDIA_SESSION_MCPTT(mcptt), &ap);
		TSK_OBJECT_SAFE_FREE(mcptt);
		va_end(ap);
		break;
	}
...
int tmedia_session_mgr_start_mbms_media(tmedia_session_mgr_t* self, const char* media_ip, const int media_port, const int media_ctrl_port, ...)
{
	tmedia_session_t* session;
	tmedia_session_audio_t* multicast_audio_session = tsk_null;
	tmedia_session_mcptt_t* mcptt = tsk_null;
	tmedia_type_t media_type = tmedia_mcptt;
	tsk_list_item_t *item;
	int ret = -1;
	...
	tsk_list_foreach(item, self->sessions){
		...
		if (tmedia_session_mgr_add_media(self, tmedia_mbms_audio) != 0) {
			TSK_DEBUG_ERROR("Could not create MBMS audio session");
			return -1;
		}

		multicast_audio_session = (tmedia_session_audio_t*)tmedia_session_mgr_find_at_index(self, tmedia_audio, 1);
		if (multicast_audio_session == tsk_null) {
			TSK_DEBUG_ERROR("Could not obtain MBMS audio session");
			return -1;
		}

		tmedia_session_mgr_set(self,
			TMEDIA_SESSION_SET_POBJECT(tmedia_mcptt, "multicast_audio_session", multicast_audio_session),
			TMEDIA_SESSION_SET_NULL());
		
//		tmedia_session_mgr_set_ro(self, sdp_ro, tmedia_ro_type_offer);

		if (mcptt->start_mbms_media)
			ret = mcptt->start_mbms_media(TMEDIA_SESSION_MCPTT(mcptt), media_ip, media_port, media_ctrl_port, &ap);

		if (ret < 0) {
			TSK_DEBUG_ERROR("MBMS media not correctly started");
			tmedia_session_mgr_remove_media(self, tmedia_mbms_audio);
			multicast_audio_session = tsk_null;
			tmedia_session_mgr_set(self,
				TMEDIA_SESSION_SET_POBJECT(tmedia_mcptt, "multicast_audio_session", multicast_audio_session),
				TMEDIA_SESSION_SET_NULL());
		}
...
int tmedia_session_mgr_set_mcptt_mbms_cbfn(tmedia_session_mgr_t* self, const void* context, tmedia_session_mcptt_mbms_cb_f fun)
{
	tmedia_session_t* session;
	tsk_list_item_t *item;
	...
	tsk_list_foreach(item, self->sessions){
		...
		tmedia_session_set_mcptt_mbms_cbfn(session, context, fun);
	}
	tsk_list_unlock(self->sessions);
...
int tmedia_session_mgr_set_mcptt_affiliation_cbfn(tmedia_session_mgr_t* self, const void* context, tmedia_session_mcptt_affiliation_cb_f fun)
{
	...
	tsk_list_lock(self->sessions);
	tsk_list_foreach(item, self->sessions){
		if (!(session = item->data) || !(session->type & tmedia_mcptt_affiliation)){
			continue;
		}
		tmedia_session_set_mcptt_affiliation_cbfn(session, context, fun);
	}
	tsk_list_unlock(self->sessions);
